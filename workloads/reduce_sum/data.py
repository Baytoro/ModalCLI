from typing import Any, Dict

import torch


def data(ctx: Dict[str, Any]) -> Dict[str, Any]:
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA not available")

    test_cfg = ctx.get("test_config", {})
    cfg = test_cfg.get("reduce_sum", test_cfg) if isinstance(test_cfg, dict) else {}

    n = int(cfg.get("n", 8_388_608))
    block_size = int(cfg.get("block_size", 256))
    warmup = int(cfg.get("warmup", 10))
    iters = int(cfg.get("iters", 50))

    if n <= 0:
        raise ValueError("n must be positive")
    if block_size <= 0 or block_size > 1024:
        raise ValueError("block_size must be in (0, 1024]")

    props = torch.cuda.get_device_properties(0)
    default_blocks = int(props.multi_processor_count) * 8
    num_blocks = int(cfg.get("num_blocks", default_blocks))
    if num_blocks <= 0:
        num_blocks = default_blocks
    if num_blocks <= 0:
        raise ValueError("num_blocks must be positive")

    x = torch.ones((n,), dtype=torch.float32, device="cuda").contiguous()

    return {
        "inputs": [x, block_size, num_blocks],
        "n": n,
        "block_size": block_size,
        "num_blocks": num_blocks,
        "warmup": warmup,
        "iters": iters,
        "dtype": "float32",
        # Streaming memory benchmark, keep default L2 flush behavior.
        "flush_l2": True,
    }
