...
template <int BLOCK_THREADS> __global__ void reduce_sum_2d_cub_kernel(const float *x, float *out, int rows, int cols) {
...
torch::Tensor reduce_sum_2d_cuda(torch::Tensor x, int64_t block_size) {
    ...
    case 1024:
        ...
        break;
    default:
        TORCH_CHECK(false, "reduce_sum_2d_cub supports block_size in {64, 128, 256, 512, 1024}, got ", threads);
    }
